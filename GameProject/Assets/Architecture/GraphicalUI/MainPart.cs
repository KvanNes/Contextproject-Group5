...
namespace GraphicalUI {
    public class MainPart : GraphicalUIPart {
		...
		private static int buttonX = Screen.width / 2;// + Screen.width / 10 * 4 ;
        private static int buttonY = Screen.height / 2 ;
        private static int buttonW = Screen.width / 8; //150;
		private static int buttonH = Screen.width / 10; //50;
        ...
        private void CreateServerButton() {
			//if (!ServerAvailable()) {
				...
				if (GUI.Button (new Rect (Screen.width, Screen.height / 2 - Screen.height /5, Screen.width / 10, Screen.width / 10), "Start server!")) {
